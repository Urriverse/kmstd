...
impl Vaddr
{
    #[inline(always)] pub const fn from_raw(r: usize) -> Self
    {
        Self(r)
    }
    ...
    #[inline(always)]
    pub const fn to_ref<'a, T>(self) -> Option<&'a T>
    {
        unsafe
        {
            self
    ...
    #[inline(always)]
    pub const unsafe fn to_ref_unchecked<'a, T>(self) -> &'a T
    {
        unsafe
        {
            self
    ...
    #[inline(always)]
    #[allow(clippy::wrong_self_convention)]
    pub const fn to_mut<'a, T>(self) -> Option<&'a mut T>
    {
        unsafe
        {
            self
    ...
    #[inline(always)]
    #[allow(clippy::wrong_self_convention)]
    pub const unsafe fn to_mut_unchecked<'a, T>(self) -> &'a mut T
    {
        unsafe
        {
            self
...
Import!
{
    pub fn MemAlloc(layout: core::alloc::Layout) -> *mut u8
    where kernel 0.1;

    pub fn MemFree(ptr: *mut u8, layout: core::alloc::Layout)
    where kernel 0.1;

    pub fn MemAllocDMA(zone: Zone, count: usize) -> Paddr
    where kernel 0.1;

    pub fn MemFreeDMA(paddr: Paddr)
    where kernel 0.1;

    pub fn MemSetSwapper(swapper: Swapper)
    where kernel 0.1;

    pub fn MemGetSwapper() -> Swapper
    where kernel 0.1;
}
